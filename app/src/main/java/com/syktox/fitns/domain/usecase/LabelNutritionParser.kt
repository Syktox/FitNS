...
import com.syktox.fitns.domain.model.MicronutrientValue
...
class LabelNutritionParser {
    ...
    fun parse(rawText: String): LabelParseResult {
        ...
        lines.forEach { line ->
            ...
            if (lower.contains("fett") || lower.contains("fat")) {
                if (lower.contains("gesättigt") || lower.contains("saturated")) {
                    if (saturatedFat == null) saturatedFat = findValueAfterDavon(line)
                } else if (!lower.contains("gesättigt")) {
    ...
    private fun findValueBeforeDavon(line: String): Double? {
        val lower = line.lowercase()
        val davonIndex = lower.indexOf("davon")
        val endIndex = if (davonIndex >= 0) davonIndex else line.length
    ...
    private fun findValueAfterDavon(line: String): Double? {
        val lower = line.lowercase()
        val davonIndex = lower.indexOf("davon")
        if (davonIndex < 0) return null
        return firstNumber(line, endIndex = line.length, startAt = davonIndex)?.value
    }
